...
class StorageBackend(Protocol):
    ...
    def upload_file(self, path: str, file_content: bytes, content_type: str = None) -> None:
        """Upload a file."""
        ...
...
class GCSStorageBackend:
    ...
    def client(self):
        if self._client is None:
            ...
            self._client = storage.Client(credentials=getattr(settings, "GS_CREDENTIALS", None))
        return self._client
...
def get_storage_backend() -> StorageBackend:
    ...
    if "gcloud" in staticfiles_backend.lower() or "GoogleCloudStorage" in staticfiles_backend:
        # Using GCS
